% This will build the para files for the frequency condition (model
% 3).  Because the words are presented in a random order every time, we
% need separate para files for each run for each participant.  
% 
% Created: 11/5/2015, bpritche

%% Initialize variables
% set up map so that we can get from word to condition number
% so nameMap{word} = <conditionNumber>
%   1 - high H, 2 - medium M, 3 - low L
% high frequency
hf_fid = fopen('high_freq.txt', 'r');
hf_words_all = textscan(hf_fid, '%s');
hf_words = hf_words_all{1};
hf_keys = ones(60, 1);
fclose(hf_fid);
% med frequency
mf_fid = fopen('med_freq.txt', 'r');
mf_words_all = textscan(mf_fid, '%s');
mf_words = mf_words_all{1};
mf_keys = ones(60, 1) .* 2;
% low frequency
lf_fid = fopen('low_freq.txt', 'r');
lf_words_all = textscan(lf_fid, '%s');
lf_words = lf_words_all{1};
lf_keys = ones(60, 1) .* 3;
% combine
all_words = [hf_words; mf_words; lf_words];
all_vals = [hf_keys; mf_keys; lf_keys];
wordFreqMap = containers.Map(all_words, all_vals);
save wordFreqMap.mat wordFreqMap
condNames = {'H', 'M', 'L'};
...
para_temp = 'IARPAlex_%s_freq_%s_rp%drn%d.para';
    ...
    for subj=1:num_subjs
        ...
        for sess_run=1:num_runs
            ...
            while ~feof(data_fid)
                ...
                condNum = wordFreqMap(lower(concept));
